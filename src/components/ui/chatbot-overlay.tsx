...
export const ChatbotOverlay = forwardRef<HTMLDivElement, { showOverlay: boolean }>(({ showOverlay }, ref) => {
    ...
    return (
        <div ref={ref} className={`bg-(--background) overflow-hidden h-164 w-132 rounded-3xl shadow-md fixed right-6 bottom-26 z-10 transition-[transform opacity] duration-500 ease-in-out ${showOverlay ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'}`}>
            <div className="flex flex-col w-full h-full p-6 gap-4">
                ...
                <div className="flex flex-row items-center justify-center gap-4 w-full h-32">
                    <textarea ref={inputRef} className="border-2 p-4 resize-none outline-none border-emerald-400 bg-emerald-50 h-full w-full rounded-xl" placeholder="Tanyakan sesuatu" />
